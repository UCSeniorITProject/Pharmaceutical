...
exports.createPrescriptionPrescribableDrug = async (req, reply) => {
  try {
    const prescriptionHistory = await sequelize.query(
			...
			`,
      {
        replacements: {
          ...
          endDate: req.body.prescriptionPrescribableDrug.prescriptionEndDate,
    ...
    if (prescriptionHistory.length === 0) {
      const prescriptionPrescribableDrug = PrescriptionPrescribableDrug.build(
      ...
      return {
        prescriptionPrescribableDrug:
          savedPrescriptionPrescribableDrug.dataValues,
      };
    } else {
...
exports.patchPrescriptionPrescribableDrug = async (req, reply) => {
  try {
    if (Object.entries(req.body.prescriptionPrescribableDrug).length === 0) {
      ...
      return {
        prescriptionPrescribableDrug: prescriptionPrescribableDrug.dataValues,
      };
    }
...
exports.getPrescriptionWithFilter = async (req, reply) => {
  try {
    ...
    return {
      prescriptionPrescribableDrugs: prescriptionPrescribableDrugs.map(
        (x) => x.dataValues
      ),
    };
  } catch (err) {
